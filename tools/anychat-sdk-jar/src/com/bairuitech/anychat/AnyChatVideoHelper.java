...
public class AnyChatVideoHelper {
	...
	public int SetVideoFmt(int userid, int streamindex, int width, int height, int pixfmt) {
		VideoRenderer r = GetRenderByUserId(userid, streamindex);
		if(r == null)
			return -1;
		try
		{
			r.CreateBitmap(width, height, pixfmt);
		}
		catch(Exception e)
		{
...
class VideoRenderer implements Callback {
    ...
    public VideoRenderer(SurfaceHolder holder) {
        ...
        int transbk = AnyChatCoreSDK.GetSDKOptionInt(AnyChatDefine.BRAC_SO_CORESDK_VIDEOBKTRANSPARENT);
        if(transbk != 0) {
        	holder.setFormat(PixelFormat.TRANSPARENT);
        	AnyChatCoreSDK.SetSDKOptionInt(AnyChatDefine.BRAC_SO_VIDEOSHOW_CBPIXFMT, AnyChatDefine.BRAC_PIX_FMT_BGR32);
        }
    }
	...
	public Bitmap CreateBitmap(int width, int height, int pixfmt) {
        if (bitmap == null) {
            try {
        ...
        if(bitmap != null && (srcRect.bottom != height || srcRect.right != width))
        	bitmap = null;
        if(bitmap == null) {
        	int AC_PIX_FMT_RGB32 = 1;
        	int AC_PIX_FMT_BGR32 = 4;
        	Bitmap.Config config = Bitmap.Config.RGB_565;
        	if(pixfmt == AC_PIX_FMT_RGB32 || pixfmt == AC_PIX_FMT_BGR32)
