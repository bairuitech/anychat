...
public class AnyChatCoreSDK
{
	...
	public static boolean	mSensorWorkFine = false;
	...
	public static AnyChatCameraHelper	mCameraHelper1 = new AnyChatCameraHelper();
	...
	public synchronized static AnyChatCoreSDK getInstance(Context context)
	{
		if(mAnyChat==null)
		{
			...
			//#define BRAC_SO_CORESDK_HWDEVICEINFO		246	///< 硬件设备信息
			String deviceString = "{\"model\":\"" + android.os.Build.MODEL + 
								...
								"\"}";
			AnyChatCoreSDK.SetSDKOptionString(246, deviceString);
			...
			mCameraHelper.SetStreamIndex(0);
			mCameraHelper1.SetStreamIndex(1);
		}
		return mAnyChat;
	}
	...
	public int CertCheck(int index, int flags) {
		int ANYCHAT_SDKCTRL_CERTCHECK = 6;
		String strInParam = "{'index':" + index + ", 'flags':" + flags + "}";
		String result = SDKControl(ANYCHAT_SDKCTRL_CERTCHECK, strInParam);
		return AnyChatUtils.GetErrorCode(result);
	}
    ...
    public void OnNotifyMsg(int dwNotifyMsg, int wParam, int lParam)
    {
		...
		case AnyChatDefine.WM_GV_CONNECT:			
			if(baseEvent != null)
				Log.e("ANYCHAT","OnNotifyMsg,wParam::"+wParam);
		...
		case AnyChatDefine.WM_GV_VIDEOSIZECHG:
			if(stateChgEvent != null)
				stateChgEvent.OnAnyChatVideoSizeChgMessage(wParam, (lParam&0xFFFF), ((lParam>>16)&0xFFFF));
			break;
		...
		case AnyChatDefine.WM_GV_VIDEOCAPCTRL:
			if(lParam==1)
				mCameraHelper1.CaptureControl(wParam==0 ? false : true);
			else
	...
	{
		int pixfmt = GetSDKOptionInt(AnyChatDefine.BRAC_SO_VIDEOSHOW_CBPIXFMT);
		mVideoHelper.SetVideoFmt(userid, 0, width, height, pixfmt);
	...
	{
		int pixfmt = GetSDKOptionInt(AnyChatDefine.BRAC_SO_VIDEOSHOW_CBPIXFMT);
		mVideoHelper.SetVideoFmt(userid, streamindex, width, height, pixfmt);
		int degree = QueryUserStateInt(userid, AnyChatDefine.BRAC_USERSTATE_VIDEOROTATION);
		int mirror = QueryUserStateInt(userid, AnyChatDefine.BRAC_USERSTATE_VIDEOMIRRORED);
