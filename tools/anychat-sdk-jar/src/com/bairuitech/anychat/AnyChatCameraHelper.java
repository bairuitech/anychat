...
public class AnyChatCameraHelper implements SurfaceHolder.Callback {
	...
	// 设置流序号
	public void SetStreamIndex(int streamid){
	...
	private void initCamera() {
		if (null == mCamera)
			return;
		try {
			AnyChatCoreSDK anychat = AnyChatCoreSDK.getInstance(mContext);
			if (bIfPreview) {
			...
			if(mStreamIndex != 0)
			{
				iSettingsWidth = anychat.GetUserStreamInfoInt(-1, mStreamIndex, AnyChatDefine.BRAC_SO_LOCALVIDEO_WIDTHCTRL);
				iSettingsHeight = anychat.GetUserStreamInfoInt(-1, mStreamIndex, AnyChatDefine.BRAC_SO_LOCALVIDEO_HEIGHTCTRL);
			}
			...
			for (int i = 0; i < fpsRange.size(); i++) {
				...
				AnyChatCoreSDK.Log("Stream:" + mStreamIndex + " Camera FrameRate: " + r[0] + " , " + r[1]);
			...
			mCamera.setPreviewCallbackWithBuffer(new Camera.PreviewCallback() {
				...
				public void onPreviewFrame(byte[] data, Camera camera) {
					if (data.length != 0 && bNeedCapture) {
						try {
							if(!AnyChatCoreSDK.mSensorWorkFine) {
								// 对天猫精灵进行方向修正
								if(android.os.Build.MODEL.equals("AILABS_S1") ||
								   android.os.Build.MODEL.equals("SCP-XIKANG-3001 series")) {
									AnyChatCoreSDK.SetSDKOptionInt(AnyChatDefine.BRAC_SO_LOCALVIDEO_ORIENTATION, 3);
								}
							}
							if(mStreamIndex != 0)
								AnyChatCoreSDK.InputVideoDataEx(mStreamIndex, data, data.length, 0, 0);
							else
			...
			if (mCamera.getParameters().getPreviewFormat() == ImageFormat.NV21)
				...
				AnyChatCoreSDK.Log("stream:" + mStreamIndex + " unknow camera privew format:" + mCamera.getParameters().getPreviewFormat());

			if(mStreamIndex != 0)
			{
				int ANYCHAT_CERTFUNC3_ANDROIDMULCAMERA = 0x00000200;
				if(anychat.CertCheck(2, ANYCHAT_CERTFUNC3_ANDROIDMULCAMERA) != 0) {
					AnyChatCoreSDK.Log("The current authorization certificate does not support multiple cameras.");
					return;
				}
				Camera.Size previewSize = mCamera.getParameters().getPreviewSize();
				anychat.SetUserStreamInfoInt(-1, mStreamIndex, AnyChatDefine.BRAC_SO_CORESDK_EXTVIDEOINPUT, 1);

				int iCurPreviewRange[] = new int[2];
				parameters.getPreviewFpsRange(iCurPreviewRange);
				AnyChatCoreSDK.SetInputVideoFormatEx(mStreamIndex, 0, mVideoPixfmt, previewSize.width, previewSize.height,
						iCurPreviewRange[1] / 1000, 0);
				anychat.SetUserStreamInfoInt(-1, mStreamIndex, AnyChatDefine.BRAC_SO_LOCALVIDEO_CAMERAFACE, cameraInfo.facing);
			}
			else
			{
				Camera.Size previewSize = mCamera.getParameters().getPreviewSize();
	...
	public void CaptureControl(boolean bCapture) {
		...
		AnyChatCoreSDK anychat = AnyChatCoreSDK.getInstance(mContext);
		if(bNeedCapture && mStreamIndex > 0)
		{
			int ANYCHAT_CERTFUNC3_ANDROIDMULCAMERA = 0x00000200;
			if(anychat.CertCheck(2, ANYCHAT_CERTFUNC3_ANDROIDMULCAMERA) != 0) {
				AnyChatCoreSDK.Log("The current authorization certificate does not support multiple cameras.");
		...
		if (bNeedCapture && mVideoPixfmt != -1) {
			try {
				...
				if(mStreamIndex != 0) {
					...
					anychat.SetUserStreamInfoInt(-1, mStreamIndex, AnyChatDefine.BRAC_SO_LOCALVIDEO_CAMERAFACE, mCameraFacing);
				} else {
					AnyChatCoreSDK.SetSDKOptionInt(AnyChatDefine.BRAC_SO_CORESDK_EXTVIDEOINPUT, 1);
					AnyChatCoreSDK.SetInputVideoFormat(mVideoPixfmt, previewSize.width, previewSize.height,
							mCamera.getParameters().getPreviewFrameRate(), 0);
			...
			if(mStreamIndex != 0)
				anychat.SetUserStreamInfoInt(-1, mStreamIndex, AnyChatDefine.BRAC_SO_CORESDK_EXTVIDEOINPUT, 0);
			else
