...
public class AnyChatDefine {
	...
	static final int ANYCHAT_SESSION_KEEP   =   WM_GV + 19;
    ...
    public static final int BRAC_SO_CORESDK_REMOTEDEBUG		=	19;	///< 远程调试（参数为int型，表示目标用户ID，复用参数长度为调试类型参数）
    ...
    public static final int BRAC_SO_LOCALVIDEO_CAMERAROTATION =	101;///< 本地摄像头旋转角度
    public static final int BRAC_SO_LOCALVIDEO_DEVICEROTATION	 =102;	///< 设备旋转角度
	public static final int BRAC_SO_LOCALVIDEO_DEVICEMODE	=	103;///< 设备类型
    public static final int BRAC_SO_LOCALVIDEO_TVFORMAT		=	104;///< 视频采集制式设置（参数为：int型，定义为DirectShow::strmif.h::AnalogVideoStandard，默认为：AnalogVideo_PAL_B）
    ...
    public static final int BRAC_SO_LOCALVIDEO_VIDEOCOLOR		=112;	///< 视频颜色控制（字符串类型，参数为JSON）
	...
	public static final int BRAC_SO_VIDEOSHOW_CBPIXFMT		=	87;	///< 视频数据回调格式（参数为int型）
    ...
    public static final int BRAC_SO_CORESDK_MEDIAUTILLIB	=	133;	///< 获取brMediaUtil.dll的库加载句柄
	...
	public static final int BRAC_SO_CORESDK_SUPPORTSIP		=	137; ///< 判断当前是否支持SIP通信
    public static final int BRAC_SO_CORESDK_SUPPORTHTML5	=   138; ///< 判断当前是否支持HTML5
	...
	public static final int BRAC_SO_CORESDK_VIDEOBKTRANSPARENT=	245; ///< 视频背景透明显示
    ...
    public static final int  BRAC_SO_UDPTRACE_NETDELAY		    =	168;	///< UDP数据包网络延迟，单位：ms，-1表示未

    // 用户多媒体流参数定义（API：BRAC_GetUserStreamInfo 传入参数）
    public static final int  BRAC_STREAMINFO_VIDEOWIDTH			=180; ///< 视频流宽度
    public static final int BRAC_STREAMINFO_VIDEOHEIGHT			=181;	///< 视频流高度
    public static final int  BRAC_STREAMINFO_VIDEOFPS				=182;	///< 视频流帧率
    public static final int  BRAC_STREAMINFO_VIDEOBITRATE			=183;	///< 视频流码率，单位：bps
    public static final int  BRAC_STREAMINFO_VIDEOCODECID			=184;	///< 视频流编码器ID
    public static final int  BRAC_STREAMINFO_VIDEOPACKLOSSRATE	=185;	///< 视频流丢包率
    public static final int  BRAC_STREAMINFO_VIDEODEVICENAME		=186;	///< 视频采集设备名称
    public static final int  BRAC_STREAMINFO_AUDIOCHANNELS		=190;	///< 音频流通道数
    public static final int  BRAC_STREAMINFO_AUDIOSAMPLERATE		=191;	///< 音频流采样率
    public static final int  BRAC_STREAMINFO_AUDIOBITRATE			=192;	///< 音频流码率，单位：bps
    public static final int BRAC_STREAMINFO_AUDIOCODECID			=193;	///< 音频流编码器ID
    public static final int  BRAC_STREAMINFO_AUDIOPACKLOSSRATE	=194;	///< 音频流丢包率
    public static final int  BRAC_STREAMINFO_AUDIODEVICENAME		=195;	///< 音频采集设备名称
    ...
    public static final int BRAC_TRANSTASK_SAVEASPATH		=	4;	///< 文件传输任务另存为路径设置，含文件名（参数为字符串TCHAR类型）
    public static final int BRAC_TRANSTASK_SOURCEFILE		=	5;///< 源文件名（含路径，参数为字符串，TCHAR类型）
    public static final int BRAC_TRANSTASK_JSONSTATUS		=	6;	///< 传输任务状态，Json字符串
    ...
    public static final int BRAC_USERSTATE_AUDIOCODECID		=	20;	///< 查询指定用户的音频编码器ID
    public static final int BRAC_USERSTATE_VIDEOCODECID		=	21;	///< 查询指定用户的视频编码器ID
    public static final int BRAC_USERSTATE_RESENDLOSSRATE		=   22;	///< 丢包补偿后的丢包率（参数为DWORD类型，返回值：0 - 100，如：返回值为5，表示丢包率为5%）
	...
	public static final int BRAC_VIDEOCALL_FLAGS_ASSISTREQ	=0x0100;	///< 请求目标用户远程协助
	public static final int BRAC_VIDEOCALL_FLAGS_CONTROLREQ	=0x0200;	///< 请求控制目标用户
    ...
    public static final int ANYCHAT_DEBUGINFOTYPE_APIINVOKE = 1;     ///< API调用日志
    public static final int ANYCHAT_DEBUGINFOTYPE_CALLBACK = 2;      ///< 回调事件日志
    public static final int ANYCHAT_DEBUGINFOTYPE_NETSTATUS = 3;     ///< 网络状态信息
    public static final int ANYCHAT_DEBUGINFOTYPE_TRANSTASK = 4;    ///< 传输任务日志
    public static final int ANYCHAT_DEBUGINFOTYPE_TRANSBUFFER = 5;   ///< 透明通道数据传输日志
    public static final int ANYCHAT_DEBUGINFOTYPE_CODEC = 6;  ///< 底层Codec信息输出
    ...
    public static final int ANYCHAT_SERVERQUERY_NAMEBYUSERID =4;	///< 根据用户ID查询用户名
    public static final int ANYCHAT_SERVERQUERY_NAMEBYSTRID	 =5;	///< 根据字符串ID查询用户名
	public static final int ANYCHAT_SERVERQUERY_USERINFOBYID =6;	///< 根据用户ID查询用户信息
    public static final int ANYCHAT_SERVERQUERY_QUEUEAGENTINFO=100;	///< 查询指定队列的坐席服务信息
    public static final int ANYCHAT_SERVERQUERY_RUNNINGSTATUS =200;	///< 查询服务器运行状态
    public static final int ANYCHAT_SERVERQUERY_ONLINEUSERS	  =201;	///< 查询服务器在线用户数
	...
	// 组播策略定义
	public static final int BRAC_MCPOLITIC_DISABLE			=	0;	///< 执行服务器路由策略，禁止所有组播发送[默认]
	public static final int BRAC_MCPOLITIC_ONLYLOCALMC		=	1;	///< 忽略服务器路由策略，只向客户端本地组播组广播媒体流
	public static final int BRAC_MCPOLITIC_SERVERANDLOCALMC	=	2;	///< 执行服务器路由策略，同时在客户端本地发送组播数据
	public static final int BRAC_MCPOLITIC_ONLYSERVERMC		=	3;	///< 忽略服务器路由策略，只向服务器本地组播组广播媒体流
	public static final int BRAC_MCPOLITIC_SERVERANDSERVERMC	=4;	///< 执行服务器路由策略，同时在服务器端发送组播数据

	// 组播功能标志定义
    public static final int BRAC_MCFLAGS_JOINGROUP		=0x00000001	;///< 加入多播组
	public static final int BRAC_MCFLAGS_LEAVEGROUP		=0x00000002;	///< 离开多播组
	public static final int BRAC_MCFLAGS_SENDDATA		=0x00000010	;///< 数据发送标志，指示该多播组用于发送数据
	public static final int BRAC_MCFLAGS_RECVDATA		=0x00000020	;///< 数据接收标志，指示该多播组用于接收数据
	...
	public static final int ANYCHAT_TSFLAGS_OVERWRITE = 0x00000100;//向服务器传送文件时 覆盖已有文件
