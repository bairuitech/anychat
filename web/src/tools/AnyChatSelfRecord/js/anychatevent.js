...
function OnAnyChatRecordSnapShot(dwUserId, lpFileName, dwParam, bRecordType) {
	...
	if (bRecordType == 0) {
		...
		// GetID("AnyChatLocalVideoDiv").setAttribute("style","background:url('file://"+lpFileName.replace(/\\/g,"/")+"')
		// no-repeat;");

	} else if (bRecordType == 1) {
		displayList("tip,record_ready,recordtime,advanceset_iframe", "none");

		displayList("advanceset_iframe,recordmsg,backRecord,Recordplay,next",
				"block");
		GetID("recordmsg").innerHTML = "视频录制完成";
		// alert("录像成功，文件路径是："+lpFileName);
		lpLocalPathName = lpFileName;
		// szUrl=szUrl.split(':');
		// lpLocalPathUrl=szUrl[0]+"://localhost:"+szUrl[2];
		lpLocalPathUrl = szUrl;

		control = false;
	}
	// BRAC_GetDmoObject("AnyChatLocalVideoDiv").removeChild(BRAC_GetDmoObject("ANYCHAT_VIDEO_LOCAL"));
}
...
function OnAnyChatLoginSystem(dwUserId, errorcode) {
	// DisplayLoadingDiv(false);
	// AddLog("OnAnyChatLoginSystem(userid=" + dwUserId + ", errorcode=" +
	// errorcode + ")", LOG_TYPE_EVENT);
	if (errorcode == 0) {
