...
//视频参数界面下拉框
var mDefaultCodeRate = ["质量模式", "40", "60", "100", "150", "200", "300", "400", "500", "600", "800", "1000", "1200", "1500"]; // 码率下拉框
var mDefaultQuality = ["较差质量", "一般质量", "中等质量", "较好质量", "最好质量"]; // 质量下拉框
var mDefaultResolution = ["176x144", "320x240", "352x288", "640x480", "720x480", "720x576", "800x600", "960x720", "1024x576", "1280x720", "1280x1024", "1920x1080"]; // 分辨率下拉框
var mDefaultFrame_rate = ["5 FPS", "8 FPS", "12 FPS", "15 FPS", "20 FPS", "25 FPS", "30 FPS"]; // 帧率下拉框
var mDefaultPreinstall = ["1", "2", "3", "4", "5"]; // 预设下拉框
var mDefaultSpeakmode = ["发言模式(默认)", "放歌模式", "卡拉OK模式", "线路输入模式"]; // 音频模式下拉框
...
function InitAdvanced() {
    filltheselect("code_rate", mDefaultCodeRate); // 填充码率下拉框
    filltheselect("quality", mDefaultQuality); // 填充质量下拉框
    filltheselect("distinguishability", mDefaultResolution); // 填充分辨率下拉框
    filltheselect("frame_rate", mDefaultFrame_rate); // 填充帧率下拉框
    filltheselect("preinstall", mDefaultPreinstall); // 填充预设下拉框
    filltheselect("Speak_Mode", mDefaultSpeakmode); // 发言模式下拉框
...
function SettingBtnMouseout(id) {
    if (Getdmo(id).getAttribute("clickstate") == "false") // 没有被点击的按钮改变背景色
...
function SettingBtnClick(id, dd) {
    ...
    Getdmo(dd).setAttribute("clickstate", "true"); // 设置被点击的按钮状态为被点击
    Getdmo(dd).style.backgroundColor = "White"; // 设置按钮背景色
    Getdmo(id).style.display = "block"; // 显示按钮对应的参数界面
}
...
function ChangeTheResult(id) {
    switch (id) {
        case "ServerSetting": // 服务器配置参数按钮
            ...
            var SelectTag = Getdmo("Video_Parameter_Interface").getElementsByTagName("select"); // 取得 音频参数设置 界面 所有select标签
            if (Getdmo("ServerSetting").checked == true) { // 标签点击事件
                ...
                for (var i = 0; i < SelectTag.length; i++) { // 循环标签
                    if (SelectTag[i].id != "quality") { //质量 下拉框不随复选框选中而启用
                        ...
                        if (Getdmo("code_rate").options[Getdmo("code_rate").selectedIndex].text == "质量模式")
                            Getdmo("quality").disabled = "";
                    }
                    for (var j = 0; j < GetAControl.length; j++)
