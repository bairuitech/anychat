...
void CALLBACK TransBuffer_CallBack(DWORD dwUserid, LPBYTE lpBuf, DWORD dwLen, LPVOID lpUserValue)
{
	...
	{
		CString strNotify;
		strNotify.Format(_T("TransBuffer_CallBack:dwUserid-%d, bufSize-%d"),dwUserid,dwLen);
...
void CALLBACK TransBufferEx_CallBack(DWORD dwUserid, LPBYTE lpBuf, DWORD dwLen, DWORD wParam, DWORD lParam, DWORD dwTaskId, LPVOID lpUserValue)
{
	...
	if(pDemoDlg)
	{
		...
		CString strNotify;
		strNotify.Format(_T("TransBufferEx_CallBack:dwUserid-%d, bufSize-%d,verify:%s"),dwUserid,dwLen,bSuccess ? _T("Success") : _T("Fail"));
		pDemoDlg->AppendLogString(strNotify);
	}	
}
// 文件传输回调函数定义
void CALLBACK TransFile_CallBack(DWORD dwUserid, LPCTSTR lpFileName, LPCTSTR lpTempFilePath, DWORD dwFileLength, DWORD wParam, DWORD lParam, DWORD dwTaskId, LPVOID lpUserValue)
{
	...
	{
		CString strNotify;
		strNotify.Format(_T("TransFile_CallBack:dwUserid-%d, lpFileName-%s, lpTempFilePath-%s"),dwUserid,lpFileName,lpTempFilePath);
		pDemoDlg->AppendLogString(strNotify);
	}
}
...
void CALLBACK RecordSnapShot_CallBack(DWORD dwUserid, LPCTSTR lpFileName, DWORD dwParam, BOOL bRecordType, LPVOID lpUserValue)
{
	...
	{
		CString strNotify;
		strNotify.Format(_T("%s CallBack:dwUserid-%d, FilePathName-%s"),bRecordType ? _T("Record") : _T("SnapShot"), (int)dwUserid,lpFileName);
		pDemoDlg->AppendLogString(strNotify);
	}
}
...
void CALLBACK SDKFilterData_CallBack(LPBYTE lpBuf, DWORD dwLen, LPVOID lpUserValue)
{
	...
	{
		CString strNotify;
		strNotify.Format(_T("SDK Filter CallBack:%s"),lpBuf);
		pDemoDlg->AppendLogString(strNotify);
	}
}
...
BOOL CBRAnyChatSDKDemoDlg::OnInitDialog()
{
	...
	m_iPort = 8906;
	m_iRoomId = 1;
	SetDlgItemText(IDC_IPADDRESS_SERVER,_T("demo.anychat.cn"));
	SetDlgItemText(IDC_EDIT_USERNAME,_T("AnyChat5"));
	UpdateData(FALSE);
...
void CBRAnyChatSDKDemoDlg::OnButtonInit() 
{
	...
	CString logstr;
	logstr.Format(_T("AnyChat Core SDK Version:%d.%d(%s)"),dwMainVer,dwSubVer,szCompileTime);
	...
	TCHAR szCoreSDKPath[MAX_PATH] = {0};
	GetModuleFileName(NULL,szCoreSDKPath,sizeof(szCoreSDKPath));
	(_tcsrchr(szCoreSDKPath,_T('\\')))[1] = 0; 
	BRAC_SetSDKOption(BRAC_SO_CORESDK_PATH,(const char*)szCoreSDKPath,_tcslen(szCoreSDKPath));
...
void CBRAnyChatSDKDemoDlg::OnButtonLogin() 
{
	...
	{
		AfxMessageBox(_T("Invalid  server ip address or port!"));
		return;
	}
	if(strUserName.IsEmpty())
	{
		AfxMessageBox(_T("Must input username for login system!"));
		return;
	}
...
LRESULT CBRAnyChatSDKDemoDlg::OnGVClientLogin(WPARAM wParam, LPARAM lParam)
{
	...
	if(lParam == 0)
	{
		...
		logstr.Format(_T("登录服务器成功，自己的用户编号为：%d(%s)"),wParam,username);

		// 将用户的ID保存在第一个位置，否则在回调函数里面可能找不到绘制的位置
		m_iUserID[0] = dwUserID;
	}
	else
		logstr.Format(_T("登录服务器失败，出错代码为： %d "),lParam);
...
LRESULT CBRAnyChatSDKDemoDlg::OnGVClientEnterRoom(WPARAM wParam, LPARAM lParam)
{
	...
	if(lParam == 0)
	{
		logstr.Format(_T("成功进入编号为：%d 的房间"),roomid);
		...
		logstr.Format(_T("申请进入房间失败，出错代码为： %d "),lParam);
...
LRESULT CBRAnyChatSDKDemoDlg::OnGVClientOnlineUser(WPARAM wParam, LPARAM lParam)
{
	...
	logstr.Format(_T("在编号为：%d 的房间共有 %d 位在线用户"),lParam, onlinenum);
	AppendLogString(logstr);
	...
	for(INT i=0; i< (INT)dwUserNum; i++)
	{
		...
		if(site != -1)
		{
			...
			logstr.Format(_T("用户%d 的视频状态是： %d"),userid,dwState);
			AppendLogString(logstr);
...
LRESULT CBRAnyChatSDKDemoDlg::OnGVClientUserAtRoom(WPARAM wParam, LPARAM lParam)
{
	...
	logstr.Format(_T("编号为：%d 的用户(%s) %s 房间"),userid,username, bEnter ? _T("进入") : _T("离开"));
	AppendLogString(logstr);
...
LRESULT CBRAnyChatSDKDemoDlg::OnGVClientMicStateChange(WPARAM wParam, LPARAM lParam)
{
	...
	logstr.Format(_T("编号为：%d 的用户 %s 本地Mic设备"),wParam, lParam ? _T("打开") : _T("关闭"));
	AppendLogString(logstr);
...
LRESULT CBRAnyChatSDKDemoDlg::OnGVClientChatModeChange(WPARAM wParam, LPARAM lParam)
{
	...
	{
		logstr.Format(_T("自己（%d ）的聊天模式改变，当前聊天模式为：%s"),userid, chatmode == 0 ? _T("公聊模式") : _T("私聊模式"));
	}
	else
		logstr.Format(_T("编号为：%d 的用户聊天模式改变，当前聊天模式为：%s"),userid, chatmode == 0 ? _T("公聊模式") : _T("私聊模式"));
...
{
	BRAC_ShowLVProperty(m_hWnd,_T("AnyChat 本地视频画质调节"));
	...
	case REFRESH_TRANSTASK_STATUS_TIMER:	// 刷新缓冲区传输任务状态
		{
			...
			if(ret != GV_ERR_SUCCESS)
			{
				KillTimer(REFRESH_TRANSTASK_STATUS_TIMER);
				m_dwTransTaskId = -1;
				SetDlgItemText(IDC_STATIC_FILESTATUS,_T(""));
				if(ret == GV_ERR_TRANSBUF_NOTASK)
					AppendLogString(_T("传输任务不存在，可能对方已离开房间！"));
				else
					AppendLogString(_T("查询任务信息失败！"));
				break;
			}
			...
			switch(dwStatus)
			{
			case 1:		strStatus = _T("Ready");	break;
			case 2:		strStatus = _T("Process");	break;
			case 3:		strStatus = _T("Finish");	break;
			case 4:		strStatus = _T("Cancel");	break;
			case 5:		strStatus = _T("Reject");	break;
			default:	strStatus = _T("Unkonw");	break;
			}
			if(dwStatus == 3)		// finish
			{
				KillTimer(REFRESH_TRANSTASK_STATUS_TIMER);
				m_dwTransTaskId = -1;
				AppendLogString(_T("传输任务已完成！"));
			}
			CString strBitrate;
			if(dwBitrate >= 1000 * 1000)
				strBitrate.Format(_T("%.02f Mbps"),(float)dwBitrate/1000/1000);
			else if(dwBitrate >= 1000)
				strBitrate.Format(_T("%.02f Kbps"),(float)dwBitrate/1000);
			else
				strBitrate.Format(_T("%.02f bps"),(float)dwBitrate);

			CString strNotify;
			strNotify.Format(_T("%s-%.02f%%-%s"),strStatus,dbProgress,strBitrate);
...
void CBRAnyChatSDKDemoDlg::OnButtonSendFile() 
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		{
			CString strError;
			strError.Format(_T("创建文件传输任务失败，出错代码:%d"),ret);
...
void CBRAnyChatSDKDemoDlg::OnButtonTransbuf() 
{
	...
	{
		CString strError;
		strError.Format(_T("创建缓冲区传输任务失败，出错代码:%d"),ret);
		AppendLogString(strError);
		SetDlgItemText(IDC_STATIC_FILESTATUS,_T(""));
	}
	// 缓冲区调用完成后， 可以立即释放掉
