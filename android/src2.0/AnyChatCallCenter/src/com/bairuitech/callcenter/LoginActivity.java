...
public class LoginActivity extends Activity implements AnyChatBaseEvent,
	...
	private void Login() {
		...
		if(TextUtils.isEmpty(mEditIp.getText().toString().trim())){
			BaseMethod.showToast(this.getString(R.string.str_serveripinput),this);
			return;
		}

		if(TextUtils.isEmpty(mEditPort.getText().toString().trim())){
			BaseMethod.showToast(this.getString(R.string.str_serverportinput),this);
			return;
		}
         ...
         */
        configEntity.ip = mEditIp.getText().toString().trim();
        configEntity.port = Integer.parseInt(mEditPort.getText().toString().trim());
        ConfigService.SaveConfig(this,configEntity);

        this.anychat.Connect(mEditIp.getText().toString().trim(), Integer.parseInt(mEditPort.getText().toString().trim()));
        /***
