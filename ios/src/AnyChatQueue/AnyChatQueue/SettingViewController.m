...
- (void)setupUI {
    ...
    clearButton.frame = CGRectMake(SCREEN_WIDTH/2 - 120, 20, 110, 40);
    ...
    doneButton.frame = CGRectMake(SCREEN_WIDTH/2 + 10, 20, 110, 40);
...
- (void)loadUrls{
    ...
    if (self.appid.length == 0 && [self.anyChatIP isEqualToString:kAnyChatCloudIP]) {
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    if (textField.tag == 0) {
        ...
        if ([self.anyChatIP isEqualToString:kAnyChatCloudIP]) {
            self.appid = kAnyChatAppId;
        } else {
            self.appid = nil;
        }
