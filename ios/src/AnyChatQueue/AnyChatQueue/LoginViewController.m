...
@interface LoginViewController ()<UITextFieldDelegate, UIActionSheetDelegate, AnyChatNotifyMessageDelegate>
...
- (void)setup {
    ...
    self.name.text = username ? username : kAnyChatUserName;
...
- (IBAction)autoRouteAction:(UISwitch *)sender {
}
...
- (IBAction)loginAction:(UIButton *)sender {
    ...
    if(appID.length == 0 && [ip isEqualToString:kAnyChatCloudIP]) {
        appID = kAnyChatAppId;
    }
     ...
     */
    [AnyChatPlatform Login:self.name.text :self.passWord.text];
...
- (void) OnAnyChatLogin:(int) dwUserId : (int) dwErrorCode {
    if (dwErrorCode == 0) {
        ...
        //
        //[[NSUserDefaults standardUserDefaults]setObject:kAnyChatIP forKey:@"IP"];
        //[[NSUserDefaults standardUserDefaults]setObject:kAnyChatPort forKey:@"PORT"];
...
- (void) OnAnyChatEnterRoom:(int) dwRoomId : (int) dwErrorCode {
    NSLog(@"用户进入房间, remoteUserId:%d", [AnyChatQueueDataManager getInstance].remoteUserId);
    [MBProgressHUD hideHUD];
    if (dwErrorCode == 0) {
