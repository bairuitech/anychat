...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self setUIControls];
    if (!self.topHeaderView.hidden) {
        self.navigationItem.title = @"登录";
        self.navigationItem.leftBarButtonItem = nil;
    }
}

- (void)setBackLisetBtn {
   
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    [button addTarget:self action:@selector(backLisetBtnClick) forControlEvents:UIControlEventTouchUpInside];
...
-(void)backLisetBtnClick{
    UIAlertController *alertVc = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"是否离开房间" preferredStyle:UIAlertControllerStyleAlert];
...
- (void) OnLogout {
    ...
    self.theStateInfo.text = @"Not connect to the server";
...
- (void) OnAnyChatEnterRoom:(int) dwRoomId : (int) dwErrorCode {
    ...
    NSString * homeStr = [NSString stringWithFormat:@"房间号:%d",dwRoomId];
    self.navigationItem.title = homeStr;
    [self setBackLisetBtn];
    [self.onLineUserTableView reloadData];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIImageView *identifyView = (UIImageView *)[Cell viewWithTag:kUserSelfValueTag];
    nameLabel.text = name;
    if (self.theMyUserID == userID) {
        bgView.image = [UIImage imageNamed:@"user"];
        identifyView.hidden = NO;
    } else {
        bgView.image = [UIImage imageNamed:@"client_user"];
    }
