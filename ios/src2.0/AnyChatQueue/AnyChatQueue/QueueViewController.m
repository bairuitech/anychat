...
- (void) OnAnyChatVideoCallEventCallBack:(int) dwEventType : (int) dwUserId : (int) dwErrorCode : (int) dwFlags : (int) dwParam : (NSString*) lpUserStr{
    ...
    switch (dwEventType) {
        ...
        {
            NSString *message = [NSString stringWithFormat:@"客服%@请求与您视频通话，是否接受？", serviceName];
            self.requestAlertView = [[UIAlertView alloc] initWithTitle:@"来电中..." message:message delegate:self cancelButtonTitle:nil otherButtonTitles:@"接受",@"拒绝" ,nil];
        ...
        case BRAC_VIDEOCALL_EVENT_REPLY:// 呼叫请求回复 2
        {
            switch (dwErrorCode)
            {
                ...
                {
                    NSString *message = [NSString stringWithFormat:@"等待客服%@回应，请稍候...", serviceName];
                    self.requestAlertView = [[UIAlertView alloc] initWithTitle:@"呼叫中..."
                                                                          message:message
                                                                         delegate:self
...
- (void)hideRequestAlertView {
    if (self.requestAlertView) {
